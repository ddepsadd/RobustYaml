...
| `RobustCategoryDocumentationProvider` | ховер по id категории: имя, описание и флаги объявления |
...
- [ ] Ховер по любому id прототипа — та же механика, что у категорий, но набор ключей у `entity`
      измеряется десятками, и без отбора попап станет нечитаемым
