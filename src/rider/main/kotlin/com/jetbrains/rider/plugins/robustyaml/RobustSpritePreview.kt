...
object RobustSpritePreview {
    ...
    fun loadImage(png: VirtualFile): BufferedImage? {
        val source = runCatching { png.inputStream.use { ImageIO.read(it) } }.getOrNull() ?: return null
        return magnify(cropFirstFrame(source, frameSize(png.parent)))
    }
    ...
    private fun magnify(image: BufferedImage): BufferedImage {
        val longest = maxOf(image.width, image.height).coerceAtLeast(1)
        val factor = (TARGET_SIZE / longest).coerceIn(1, MAX_FACTOR)
