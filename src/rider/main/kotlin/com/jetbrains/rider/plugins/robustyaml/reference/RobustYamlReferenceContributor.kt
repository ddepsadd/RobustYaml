...
import com.jetbrains.rider.plugins.robustyaml.navigation.RobustDeclarationTarget
...
private object LocalizationIdReferenceProvider : PsiReferenceProvider() {
    override fun getReferencesByElement(
        ...
        if (!isLocalizationValue(scalar)) return PsiReference.EMPTY_ARRAY
 ...
 *
 * It lives beside its only caller and not in `RobustYamlContext`, where it used to: everything
 * else there is a walk of the tree that needs neither project nor index, which is what lets the
 * whole file be tested under a bare `ParsingTestCase`. This one asks the backend for the type of
 * the field and the `.ftl` index for the message, so kept in that company it would have cost the
 * shared vocabulary both its testability and its place under every other package.
 */
private fun isLocalizationValue(scalar: YAMLScalar): Boolean {
...
private object PrototypeIdReferenceProvider : PsiReferenceProvider() {
    override fun getReferencesByElement(
        ...
        val scalar = element as? YAMLScalar ?: return PsiReference.EMPTY_ARRAY
        if (!namesPrototypeId(scalar)) return PsiReference.EMPTY_ARRAY
        return arrayOf(PrototypeIdReference(scalar))
    }

    /**
     * Five key names have always been read as references, and the typed fields the backend knows
     * about were left out — `prototypes:` under a spawner, `result:` of a recipe, `graph:` of a
     * construction. Ctrl+click missed them, and worse, so did rename: the declaration and the five
     * names were rewritten while 17132 values of ss14-wega kept the old id, with nothing to say so
     * until the validator caught up on a warm backend.
     *
     * The kind is not asked for here, and asking the backend would be wrong rather than slow: it
     * answers about types somebody has already requested, so the same rename would rewrite a
     * different set of files depending on what the daemon happened to have warmed. The index
     * answers the weaker question the same way every time — see [RobustDataFields.namesPrototype].
     */
    private fun namesPrototypeId(scalar: YAMLScalar): Boolean {
...
/**
 * A value naming a prototype by id. The element it sits on is not always a YAML scalar: a guidebook
 * document names prototypes from `<GuideEntityEmbed Entity="…"/>`, and an attribute value has a
 * manipulator of its own, so the same reference serves both — Ctrl+click, Find Usages and rename
 * included.
 */
class PrototypeIdReference(element: PsiElement) :
