...
class RobustRenameProcessor : RenamePsiElementProcessor() {
    ...
    override fun substituteElementToRename(element: PsiElement, editor: Editor?): PsiElement? {
        // A value handed over as itself is an item of a sequence; the declaration is found by its
        // text either way. Renaming the item would rewrite the one line under the caret and leave
        // the declaration and every other reference behind.
        val id = when {
            element is YAMLScalar -> element.textValue
            element is YAMLKeyValue && !RobustYamlContext.isPrototypeIdDeclaration(element) ->
                (element.value as? YAMLScalar)?.textValue
            else -> return element
        } ?: return null
