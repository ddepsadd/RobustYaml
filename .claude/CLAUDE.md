...
Фронтенд разложен по роли в платформе — пакет отвечает на вопрос «что это за расширение IDE»,
а не «про что оно». Найдя EP в `plugin.xml`, сразу знаешь папку.

| Пакет | Что там |
|---|---|
| `index` | `FileBasedIndex` — единственный источник фактов о чекауте |
| `lookup` | фасады над индексами с кэшем: `RobustPrototypeIndex`, `RobustDataFields`, `RobustLocalization`, `RobustResources`, `RobustMigrations`, `RobustGuidebook`, `RobustEntityLoc` |
| `highlighting` | аннотаторы, цвета, страница схемы, гуттер-пикер |
| `inspection` | `RobustValidation` и инспекции поверх неё |
| `quickfix` | `Change*Fix`, `AddRequiredFieldsFix` |
| `completion` | автокомплит и char filter |
| `reference` | контрибьюторы ссылок (YAML и гайдбук) |
| `navigation` | Find Usages, rename, target evaluator, переходы, Goto Symbol |
| `documentation` | ховеры, превью спрайтов, XML-doc, разметка |
| `editor` | обработчики Enter и дефиса, code style |
| `project` | корни, дерево, настройки, доступ к записи, проблемные файлы |
| `backend` | `RobustBackend` — клиент rd |

В корне пакета остаются двое. `RobustYamlContext` — общий словарь PSI, которым пользуются все
пакеты, и ссылка на него из любого места читается без указания папки. `RobustYamlModel.Generated.kt`
двигать нельзя вовсе: путь задан `ktOutput` в `protocol/build.gradle.kts`, и `:protocol:rdgen`
перезапишет файл там же — плюс пакет у него свой, `com.jetbrains.rd.ide.model`.

Тесты остались одним пакетом: их 23, каждый назван по тому, что проверяет, и раскладывать их
по тем же папкам значило бы держать структуру в двух местах.

**`grep` по `RobustDataFieldIndex.kt` молча ничего не находит.** Страж конца текста в `classScopes`
(`val next = … else <U+0000>`) записан сырым нулевым байтом вместо `' '`, и по одному этому
байту grep объявляет файл бинарным: он не говорит «совпадений нет», он вообще не печатает строки.
Полчаса уходит на выяснение, куда делся код, которого в файле полно. Ищи по нему `grep -a` или
python. Сам файл валиден, Kotlin такой литерал принимает. И, что важнее, писать этот байт в текст
руками нельзя: сюда он уже один раз попал при описании этой же истории — и в бинарный превратился
уже дневник, вместе со всеми своими диффами.

Помни про три вещи при переносе файлов. В `plugin.xml` классы записаны полным именем — там их 48,
и опечатка не ловится компиляцией, только запуском; проверять надо существованием `.class` в
`build/classes/kotlin/main`. Замеры зовут код рефлексией по имени, поэтому имя пакета в них не
зашито: `MeasureReferences.pluginClass(simpleName)` перебирает пакеты сам. А вот категорию логов
(`-Didea.log.debug.categories=#com.jetbrains.rider.plugins.robustyaml`) трогать не пришлось, и это
проверено по байткоду, а не по памяти: `LogLevelConfigurationManager` берёт по строке категории
обычный `java.util.logging.Logger` и зовёт `setLevel`, а JUL наследует уровень по точкам — уровень,
выставленный на корневом пакете, действует на все подпакеты.
...
| `RankedFix` | ранг подсказки, видимый платформе: `Comparable` для тултипа, `PriorityAction` для попапа |
| `RewriteContent` | правка значения манипулятором и каретка за ней — сама платформа её не двигает |
...
| `RobustYamlValueIndex` | `FileBasedIndex` по `.yml`, гайдбуку и `.cs`: значения-кандидаты для Find Usages |
| `RobustCodeLinks` | что литерал `.cs` называет в контенте: путь, id прототипа, состояние спрайта |
| `RobustCodeUsages` | ссылка на id в файле без парсера, обход таких ссылок и вид по `ProtoId<X>` |
| `RobustCodeDeclarationHandler` | Ctrl+клик с литерала `.cs`, когда ссылки на нём нет |
| `RobustCodeReferenceContributor` | ссылки на `CSharpStringLiteralExpression`: путь, состояние, id |
| `RobustCodeCompletionContributor` | подрезает `DUMMY_IDENTIFIER` внутри литерала `.cs` |
| `RobustCodeDocumentationProvider` | ховер по литералу `.cs`: спрайт, `.rsi`, сущность, сообщение |
...
| `RobustFileWritingAccess` | разрешает запись в прототипы, `Locale` и гайдбук вне content root |
| `RobustGuidebook` | что в `ServerInfo/**/*.xml` считается ссылкой на прототип |
| `RobustGuidebookReferenceContributor` | ссылка на id прототипа в атрибуте embed'а гайдбука |
| `RobustGuidebookAnnotator` | ошибка на embed, чей прототип не объявлен или другого вида |
...
| `RobustLocaleUsageIndex` | `FileBasedIndex` по `.cs`, `.ftl`, `.xaml`, `.xml`: где ключ используется |
| `RobustLocaleAffixIndex` | `FileBasedIndex`: начала и концы ключей, которые собираются в коде |
| `RobustUnusedLocalizationAnnotator` | серое предупреждение на сообщение `.ftl`, которого никто не просит |
...
| `RobustLocaleRenameHandler` | Shift+F6 по ключу в `.ftl` и гайдбуке: цель берётся по offset каретки |
| `RobustLocaleNameValidator` | какое имя считается ключом в диалоге переименования |
| `RobustLocaleNavigation` | Ctrl+клик по ключу в `.ftl` и `.cs`, Alt+F7 внутри `.ftl` |
...
| `RobustSpriteGutter` | кадр спрайта в гуттере рядом со строкой, которая его называет |
| `RobustInlayHints` | инлеи: имя сущности после id, текст сообщения после ключа локализации |
| `RobustPrototypeFolding` | сворачивание объявления в `вид id` и компонента — в его имя |
| `RobustPrototypeStructureView` | Structure View: объявления файла и компоненты под ними |
| `RobustTemplateContext` | где раскрываются шаблоны плагина: `.yml` под корнями прототипов |
| `RobustSequenceDashMacro` | `- ` в шаблоне пишется, только если строка его ещё не несёт |
| `RobustSprites` | какой `.rsi` управляет `state:` и что этот `.rsi` объявляет |
| `RobustInvalidSpriteStateInspection` | ошибка на состояние, которого нет в спрайте |
| `RobustPrototypeHierarchy` (lookup) | предки и потомки прототипа; вниз — через индекс ссылок |
| `RobustPrototypeHierarchyProvider` | Ctrl+H по id прототипа: дерево наследования в обе стороны |
...
**Имена компонентов берутся из имён файлов, а не из PSI.** Весь анализ .NET живёт в бэкенде за
rd-протоколом: у фронтенда нет ни резолва, ни типов, ни атрибутов. Эвристика `SpriteComponent.cs` →
`Sprite` промахивается на `[ComponentProtoName]` (17 файлов из 2414 в ss14-wega).

Формулировка «парсера нет вовсе» при этом устарела. В 2026.2 объявлен
`lang.parserDefinition language="C#" → CSharpKotoParserDefinition`, и дерево строится настоящее:
у строкового литерала есть свой класс `CSharpStringLiteralExpression` с `getContentRange()` и
`getLiteralType()` (все восемь форм, включая raw и интерполированные), к нему зарегистрирован
`lang.elementManipulator`, а сам Rider вешает на него ссылки —
`psi.referenceContributor language="C#"` с `CSharpWebReferenceContributor`. Синтаксис фронтенду
доступен, семантика нет; на выводы про имена компонентов это не влияет, а вот ссылки, автокомплит
и ховер внутри литерала на этом и построены.
...
**Аргументы того же атрибута нельзя читать от конца совпадения.** Регекс выше съедает `("graph"`
вместе с именем, поэтому `argumentsOf(text, match.range.last)` упирается в запятую, не находит
открывающей скобки и возвращает пустую строку: у любого `[DataField("x", required: true)]` флаг
`required` не виден вовсе. Заметить это было нечем — флаг читается только как «да», а его
отсутствие ничего не ломает. Разбор начинается от конца **имени** атрибута, с пропуском
необязательного суффикса `Attribute`; после фикса компонентов с обязательными полями стало 358
вместо 271, и нарушений на ss14-wega по-прежнему 0.

**Сериализатор прототипа опознаётся по `PrototypeId` в имени, а не по суффиксу `Serializer`.**
Бэкенд матчит суффикс и не ошибается, потому что следом требует `[Prototype]` у найденного типа
(`KindOf`); индекс такого вопроса задать не может — это другой файл. Без более строгого имени
`ConstantSerializer<DrawDepthTag>` и `FlagSerializer<CollisionLayer>` проходят за виды прототипов,
и одни только `drawdepth:` (398 значений), `mask:` (594) и `airBlockedDirection:` (29) попадают
под rename, которому там делать нечего. Все семь настоящих сериализаторов несут в имени
`PrototypeId`, а 250 применений `TimeOffsetSerializer`, `ResPathSerializer` и прочих — нет.
...
версии Gradle-плагина и не обновлялся — логи и подложенная dll бэкенда там протухшие, диагностика
по ним врёт. Актуальный `idea.log` и `plugins/ReSharperPlugin.RobustYaml/dotnet/*.dll` — в первом.
Протухшие 306 МБ удалены; если каталог появится снова, значит его создала не текущая сборка.

**Предел обхода и брошенное исключение — это два способа соврать «я всё знаю».** `Collect`
останавливался на `MaxTypes` и отдавал собранное через `Resolved`, то есть с флагом, по которому
фронтенд кладёт ответ в `ready` до ближайшей правки `.cs`: недосчитанные поля становились бы
«неизвестными ключами» на всю сессию, причём молча и без переспроса. Теперь полнота обхода едет
наружу (`out bool complete`), и неполный ответ уходит как `Unbuilt` — ровно то же правило, что уже
действовало для `InheritorLimit`. Замер: самая широкая иерархия ss14-wega — 10 типов из 32, то есть
предел сегодня не достигается, но перестал быть тихим. Вторым тем же самым была голая граница
эндпоинта: `owner.Type` в неявном универсальном контексте роняет NRE, фронтенд ловит сбой,
убирает ключ из кэша — и тип спрашивается заново на каждом проходе демона, каждый раз с полным
поиском по символам. `try/catch` возвращает `Unbuilt`, отмена (`OperationCanceledException`)
пробрасывается, потому что она не ответ. Заодно: список наследников стал множеством
(`EntityPrototype` объявлен четырежды, автокомплит показывал дубли) и сортируется, а `PrototypeKinds`
снимает `Unlift()` как все соседи.

**ReSharper-пакет собирался пустым и всё равно публиковался.** В RESHARPER-сборку попадают только
маркер зоны и сгенерированная rd-модель — весь `RobustYamlHost` под `#if RIDER`, — то есть в
Visual Studio ставился плагин, который не делает ничего, а `pushNuGet` висел на
`finalizedBy(publishPlugin)`, то есть отправлял его **после** уже отгруженного zip: упади токен, и
релиз оказывался опубликован наполовину. Теперь `IsPackable=False`, `packDotNet` снят с
`finalizedBy(buildPlugin)`, `pushNuGet` — с публикации; обе задачи остались зарегистрированными,
вернуть — одна строка. Проект при этом **продолжает собираться**, и это не инерция: он сторожит
границу `#if RIDER`, потому что Rider SDK там не подключён и любое Rider-only обращение мимо
директивы падает именно здесь.

**Версии платформы держались в двух файлах и синхронизировались комментарием.** Фронтенд берёт
`ProductVersion` из `gradle.properties`, бэкенд — `SdkVersion` из `src/dotnet/Plugin.props`, и
формы у них разные (`2026.2` против `2026.2.0.1`), поэтому одна из другой не выводится. Передавать
значение из Gradle отвергнуто: `Plugin.props` читается и при сборке солюшена прямо в IDE, а без
Gradle свойство пустое и восстановление пакетов ломается. Вместо этого сверка при конфигурации по
ведущей паре `<год>.<минор>`. Цена расхождения именно в том, что его не видно: `WaveVersion`
режется из `SdkVersion` строкой в `Directory.Build.props`, бэкенд со старым SDK объявляет старую
волну, хост его не грузит — и ховеры с автокомплитом молчат ровно так же, как молчат на непрогретом
бэкенде. Сторож проверен обломом: подменённый `2026.3.0.1` роняет конфигурацию с текстом про оба
файла, возврат — снова тишина.

**`FindInheritors` помечен `[Obsolete]`, и это нашёл компилятор, а не ревью.** Текст причины —
«base type substitution applied to a derived class has no sense»; нам она не мешает, потому что
подстановка у результата не читается, берётся только `GetTypeElement().ShortName`. Неустаревшую
перегрузку офлайн найти не удалось: ни XML-доков, ни самой `JetBrains.ReSharper.Psi.Search.dll`
в кэше NuGet и в дистрибутиве фронтенда нет. Пока живём с предупреждением.

**Раздача идёт своим репозиторием плагинов, а маркет — необязательная ветка.** `updatePlugins.xml`
на ветке `gh-pages`, zip — ассетом GitHub Release; пользователь вставляет URL в Manage Plugin
Repositories и дальше обновляется штатно. Файл собирается **из `plugin.xml`, приехавшего в zip**,
а не из исходников: версия и `since-build` там уже пропатчены `patchPluginXml`, и репозиторий,
обещающий не то, что лежит в архиве, хуже отсутствующего. `until-build` в самом `plugin.xml` не
проставляется вовсе (в собранном есть только `since-build="262"`), поэтому в описании он выводится
как `<мажор>.*`: плагин стоит на внутренних API, проверенных на одной сборке Rider, и следующий
мажор надо собрать и попробовать, а не разрешить. Заметки релиза и `<change-notes>` берутся из
одного места — первого блока `CHANGELOG.md`, — иначе диалог обновления и страница релиза говорили
бы разное. Публикация в маркет пропускается, когда секрета `PUBLISH_TOKEN` нет, а не роняет прогон.
Коммит в `gh-pages` помечен `[skip ci]`: CI объявлен на `push` без фильтра веток и иначе собирал бы
плагин на каждую публикацию xml.

**`plugin.xml` лежит в zip не сам по себе, а внутри jar — и проверка на подделанном архиве этого
показать не могла.** Первый релиз 1.0.0 собрался, тесты прошли, GitHub Release опубликовался, а
следующий шаг упал голым `StopIteration`: `next(n for n in zf.namelist() if
n.endswith('META-INF/plugin.xml'))` не нашёл ничего. `buildPlugin` кладёт в архив
`<плагин>/lib/*.jar`, дескриптор внутри jar, то есть zip надо открывать дважды —
`zipfile.ZipFile(io.BytesIO(zf.read(jar)))`. Перебираются оба jar: у `-searchableOptions`
дескриптора нет вовсе. Цена ошибки в том, **как** она была «проверена»: генератор гонялся до релиза,
но по архиву, который я собрал сам из настоящего `plugin.xml`, — то есть проверялась ветка разбора
XML и не проверялась единственная неизвестная, раскладка архива. Теперь тело шага вынимается из
самого workflow регексом и запускается по **опубликованному** ассету (`gh release download`), так
что проверяется то, что поедет.

**Каталоги-гиганты в репозиторий не попадают, а семимегабайтный `nuget.exe` попадал.**
`.gradle/` (1.6 ГБ), `.intellijPlatform/` (5.4 ГБ) и `build/` — это локальные кэши, они в
`.gitignore` с самого начала, и в истории их нет ни одного коммита (`git log --diff-filter=A`
по этим путям пуст). Клон — 170 файлов и 2.79 МиБ, из которых 7.4 МБ занимали два виндовых
бинарника из шаблона. `tools/nuget.exe` звали только `runVisualStudio.ps1` и `publishPlugin.ps1`,
а сама цепочка `.ps1` давно не работает: `settings.ps1` указывает на
`ReSharperPlugin.RobustYaml.sln`, тогда как в репозитории `.slnx`. Публикацию при этом делает
Gradle своим `pushNuGet` (`dotnet nuget push`), то есть терять нечего — удалены четыре `.ps1`,
`nuget.exe` и run-конфигурация `VisualStudio.xml`, которая их запускала. `tools/vswhere.exe`
остаётся: его зовёт `build.gradle.kts` на Windows, чтобы найти MSBuild от Visual Studio, и без
него сборка .NET-стороны в CI ушла бы на `dotnet msbuild` — другое поведение, а не чистка.

**CI гонял только `:buildPlugin` и `:testDotNet`.** 220 случаев фронтенда — тех самых, которыми
закрыто всё структурное (носитель тега, сегмент пути, отступ дефиса), — не запускались нигде, кроме
рабочей машины. Теперь `:test` стоит отдельным шагом рядом с `:testDotNet`: падение каждого из них
означает разное, и в одной строке они бы слиплись. В `Deploy.yml` обе сюиты добавлены **перед**
`:publishPlugin`, а не оставлены на CI: тег можно поставить на коммит, которого push-сборка не
видела, а публикация версии необратима.
...
**Элемент списка — не позиция ключа, а двоеточие об этом не знает.** Ключ от значения отличался
одним вопросом «есть ли двоеточие слева от каретки», и у элемента блочного списка его нет никогда:
пустой `- ` под `visMask:` считался позицией ключа, значения enum брались из `keyValues` (у не-словаря
он пуст), и автокомплит отвечал «No suggestions». Набранная буква положения не меняет — двоеточия
по-прежнему нет, — так что выглядело это как «работает через раз». Теперь подъём по родителям решает
прямо: что встретилось первым, маппинг или элемент списка. Пустой `- ` под `components:` при этом
остаётся позицией ключа, потому что текстом он неотличим (автокомплит подставляет свой идентификатор,
и элемент в обоих случаях скаляр), а отвечает на него ветка ключей — у поля-словаря значений нет.
Проверять `atKeyPosition` в одиночку осталось только ей.
...
**Живые шаблоны попадают в автокомплит сами, но контекст надо спрашивать у оригинального файла.**
`LiveTemplateCompletionContributor` (`language="any"`, `order="first"`, `id="liveTemplates"`) кладёт
применимые шаблоны в тот же список, что и все остальные, — `shouldShowAllTemplates()` читается как
`!isUnitTestMode() || TestModeFlags.is(...)`, то есть в проде он истинен (легко прочесть наоборот:
там `ifeq`, а не `ifne`). Контекст он строит от `parameters.getPosition().getContainingFile()` —
от **копии**, которую автокомплит создаёт с `DUMMY_IDENTIFIER` внутри, а `virtualFile` у копии
лёгкий и не лежит ни под одним корнем. Пока `RobustTemplateContext` спрашивал `context.file`,
шаблоны молча выпадали из списка, и на `ent` всплывали только стоковые ключи, инфиксом которых
он является (`parent`, `placement`, `components`); при открытом попапе Tab принадлежит попапу,
поэтому скелет не раскрывался. Раскрытие с закрытым попапом при этом работало всегда — там файл
настоящий. Спрашивается `originalFile`.

**Наш контрибьютор объявлен `order="first, after liveTemplates"`, и вторая половина не менее важна,
чем первая.** `first` глушит стоковый YAML-контрибьютор, а `after liveTemplates` спасает шаблоны от
нашего же `stopHere()` (он стоит в восьми ветках и останавливает всех, кто **после**). Оба
контрибьютора без id объявлены `first`, то есть порядок между ними не определён, и `comp` внутри
`components:` появлялся бы через раз — ровно там, где мы отвечаем своими ключами. Встать за
шаблонами ничего не стоит: `LiveTemplateCompletionContributor` сам прогоняет остаток цепочки через
`runRemainingContributors` и добавляет шаблоны уже после её возврата. Тем же приёмом Rider
регистрирует свой `commands`.

**Ctrl+Space шаблонов не показывает никогда, и это не наша поломка.**
`ListTemplatesHandler.filterTemplatesByPrefix` берёт шаблон, только если текст документа перед
кареткой `endsWith(template.getKey())`. На пустом префиксе не заканчивается ничем. Полный список
применимых шаблонов даёт Ctrl+J — то самое действие, из которого фильтр и позаимствован.

**Дефис в шаблоне — переменная, а не литерал.** Enter под `components:` уже оставляет `- `
(`RobustSequenceEnterHandler`), и шаблон `comp`, начинавшийся с `- type:`, давал `- - type:` —
поломка, которую устроили две фичи плагина, согласившись друг с другом. Живой шаблон не умеет
говорить «эта часть иногда», поэтому дефис вычисляет макрос (EP `liveTemplateMacro`,
`RobustSequenceDashMacro`): он читает **текст** строки слева от `templateStartOffset` и идёт назад
через пробелы — нашёл дефис, значит элемент уже открыт. PSI там спрашивать нечего: на момент
раскрытия аббревиатура удалена, а текст ещё не вставлен. Та же мина была у `ent` и `proto`.

**`hasBackground = false` у декларативного инлея — это не «без фона», а «красить как текст».**
Перегрузка с `boolean` — обёртка над настоящим методом: `true` даёт `HintFormat.default`, `false` —
`HintFormat.default.withColorKind(TextWithoutBackground)`. Отсюда подсказка читалась продолжением
значения, а не аннотацией над ним. Формат задан явно: `HintColorKind.Default` (единственный вид,
который не исключение — `hasBackground()` в байткоде это «всё, кроме `TextWithoutBackground`»),
шрифт `ABitSmallerThanInEditor` и `OnlyPadding`. Внешний отступ (`MarginAndSmallerPadding`) пробовали
и убрали: он читается как зазор между значением и подсказкой, то есть как раз то, чего у подсказки
быть не должно. Цвет при этом остаётся схемным — мы говорим «это подсказка», красит тема.

**Якорь инлея — конец диапазона скаляра, и никаких обходов пробелов.** Хвостовые пробелы в
`textRange` плоского скаляра не входят (`RobustInlayAnchorTest` это и закрепляет: у `id: X `
последний символ диапазона — `X`), поэтому обход вперёд «до конца строки» ставит чип **за** пробелом
и делает ровно ту дырку, которую был призван убрать: на `station_beacon.yml:3`, где пробел
авторский, подсказка уезжала на колонку. Симптом лечения оказался неотличим от симптома болезни.
`relatedToPrevious = true` сохраняется: он уходит в `addInlineElement(offset, relatesToPrecedingText,
…)`, а тот кладёт маркер со `stickingToRight`, то есть при дописывании id буквы появляются слева от
подсказки, а не справа. Ценой остаётся один кадр дёрганья при наборе **хвостового** пробела, когда
маркер уехал, а пересчёт вернул его назад.

**`EndOfLinePosition` этот кадр убирает, и всё равно отвергнута.** Позиций во фреймворке три
(`InlineInlayPosition`, `EndOfLinePosition`, `AboveLineIndentedPosition`), и вторая привязана к
**номеру строки**, то есть к сдвигам внутри строки невосприимчива. Но точку вставки она берёт как
`Plan$Eol.getLineEndOffset()` → `addAfterLineEndElement`, то есть после всей строки — включая
хвостовой комментарий. Замер: строк с id-подобным ключом в прототипах 47376, с хвостовым
комментарием 805 (1.70%), и комментарии там длинные (`id: AirAlarmElectronics # ConstructionGraph:
AirAlarm`) — подсказка уезжала бы на десятки символов от значения. Меняем артефакт длиной в кадр,
возникающий при вводе бессмысленного в YAML символа, на постоянно неверное положение в 805 строках.

**Гибрид «где нет комментария — end-of-line» не работает по построению.** Конец строки считается
**после** хвостовых пробелов, поэтому на `id: X ` без комментария гибрид вернул бы ту же дырку.
Третье условие («и нет хвостовых пробелов») выключает гибрид ровно в тот момент, когда пробел
набирают, — то есть в единственном сценарии, ради которого он затевался; вдобавок смена вида позиции
— это уничтожение одного инлея и создание другого, то есть мигание вместо сдвига на колонку.

**Гуттер рисует только тот кадр, который назван, — это не то же, что берёт ховер.**
`findImage` при промахе падает на `icon.png`, а потом на **первый PNG по порядку детей**; у
`hierophant.rsi` `icon.png` нет, поэтому в гуттер приезжал первый кадр `full.png` (лист 96×128) —
угловой тайл стены, который в 16 px выглядит обломком. Тот же случайный кадр журнал уже отверг
однажды для ховера. `gutterFrame` рисует состояние, одиночный `.png` либо `icon.png`, а каталог без
него не получает иконки вовсе — ховер по такому пути и так показывает сетку всех состояний. Замер:
путей на `.rsi` 11563, из них 4212 с соседним `state:` (не затронуты), каталогов 4515, `icon.png`
есть у 60.8%.

**Structure View берётся точкой YAML, а фолдинг — обычной.** `LanguageFolding.forLanguage`
собирает все зарегистрированные билдеры в `CompositeFoldingBuilder`, то есть вклад аддитивен и
стоковые сворачивания остаются. А `lang.psiStructureViewFactory` резолвится в **один** инстанс на
язык: взяв его, плагин овладел бы структурой всех YAML в IDE. Поэтому вход через
`com.intellij.yaml.customStructureViewFactory` (`intellij.yaml.backend.jar`) — точку, которую YAML
держит ровно для этого: `null` в ответ оставляет стоковый вид, и это же возвращается в файле без
объявлений. `PsiTreeElementBase` при этом лежит в отдельном модуле `intellij.platform.structureView`,
его надо добавить в `bundledModule`.

**Сырых нулевых байтов в исходниках больше нет.** Их было три, и все осмысленные: страж «спрошено,
ответа нет» в `RobustInlayHints`, разделитель ключа кэша `"$kind\0$id"` в `ChangePrototypeIdFix` и
страж конца текста в `classScopes` (`RobustDataFieldIndex`, единственный известный журналу). Каждый
из них делал свой файл бинарным для `grep` — тот не говорит «совпадений нет», он вообще ничего не
печатает. Все три записаны escape-последовательностью `\u0000`: значение то же байт в байт, файлы
снова текстовые (`grep` по `RobustDataFieldIndex.kt` теперь даёт 13 совпадений вместо нуля строк).
Правило простое: символ нужен — пиши escape, а не байт.
...
**Rename не трогает `migration.yml`, и это решение, а не недоделка.** Переименование id, на который
указывает правая часть какой-нибудь из 487 записей, оставляет запись смотрящей в пустоту. Дописать
туда новое имя технически несложно, но словарь — это контракт совместимости, по которому читаются
карты и на который смотрят снаружи; правкой чужого контракта рефакторинг не занимается, а решать за
автора, чем стала миграция после переименования, тем более. Что рефакторинг обязан не делать —
подсовывать эту мёртвую цель в quick fix: подсказка «переименовано в X» брала цель из словаря
не глядя, и после локального rename один битый id менялся бы на другой. Теперь цель предлагается,
только если она объявлена (и нужного вида, когда вид известен). На чекауте это ничего не меняет —
`MeasureMigrations` показывает 0 недостижимых целей, — и срабатывает только после правки у себя.
...
**Порядок подсказок платформа выбрасывает, если фикс о нём не сообщил.** На `layer: WallLaye`
первым предлагался `AllMask`, а `WallLayer` — в одной правке — прятался под `More actions`. Причина
не в ранжировании: наш список приходит верным, а тултип берёт из него первое по алфавиту. Путь
такой: `DaemonTooltipActionProvider.extractMostPriorityFixFromHighlightInfo` →
`CachedIntentions.createAndUpdateActions` → `getAllActions`, а множества там упорядочены
`IntentionActionWithTextCaching.compareTo`, у которого в байткоде две ветки — `compareTo` самого
действия, если оно `Comparable`, иначе `Comparing.compare(getText(), getText())`. Поэтому ранг
теперь виден платформе, и сообщать его приходится двумя способами сразу: `Comparable` — для
сортированного множества за тултипом, `PriorityAction` — для попапа Alt+Enter, где сортирует
`DefaultIntentionsOrderProvider.getPriorityWeight`. Фиксы, пришедшие из инспекции, до попапа едут
завёрнутыми в `QuickFixWrapper`: он `final` и не `Comparable`, зато `PriorityAction` реализует —
там работает вторая половина. Равные ранги при этом обязаны различаться: множества — `TreeSet`,
и ничья молча теряет фикс, а два плохих элемента одного списка дают два предложения ранга ноль.
Ничью разводит текст, как платформа и сделала бы сама.

**Опечатка в одном регистре не подсказывалась вовсе.** `EditDistance.levenshtein(…, false)` — это
сравнение без учёта регистра, поэтому `wallLayer` отстоит от `WallLayer` на ноль правок, фильтр
`1..limit` его выкидывает, и единственная нужная подсказка — то же имя, написанное правильно —
оказывается единственной невозможной. Для значений enum расстояние теперь считается с учётом
регистра; там, где движок сам читает без учёта (обычный enum-датафилд), такая разница ошибкой
не считается вовсе, так что терять нечего.

**Каретку после фикса ставит сам фикс, потому что документ её не двигает.** `DocumentImpl.replaceString`
перед записью срезает общий префикс и общий суффикс (два посимвольных цикла до `subSequence`),
поэтому замена `WallLaye` → `WallLayer` доезжает не заменой восьми символов, а вставкой одной буквы
`r` ровно в позицию каретки. А `CaretImpl$PositionMarker.changedUpdateImpl` перешагивает вставку
в свою же позицию только при `needToShiftWhiteSpaces`: символ слева пробельный, вставленный фрагмент
пробельный и без переводов строк. Буква не подходит ни под одно условие, и пользователь остаётся
стоять внутри слова: `WallLaye|r`. При обычной печати этого не видно, потому что там каретку двигает
само действие ввода, а не документ. Смещение считается **до** записи: начало содержимого не сдвинется
(это и есть срезанный префикс), тогда как `textRange.endOffset` у скаляра в кавычках уехал бы
за закрывающую кавычку.

Помощник (`rewriteContent`) подходит четырём фиксам из шести. `ChangeFieldNameFix` пишет `setName`,
но смещение берёт оттуда же: манипулятор `YAMLKeyValue` — это `YAMLKeyValueKeyManipulator`, и его
содержимое — сырой текст **ключа** без кавычек, то есть начало содержимого и есть начало ключа.
А `ChangeTypeTagFix` помощник взять не может вовсе: он держит носителя тега (ключ или элемент
списка), и манипулятор по нему нашёлся бы — тот самый ключевой, — так что фикс переписал бы имя
ключа на имя типа. Он и `AddRequiredFieldsFix` пишут документом и каретку ставили сами с самого
начала.

**Подсказка id ищется в списке своего вида, а не во всём индексе.** Тот же фикс для прототипов
делал ровно наоборот: Левенштейн по всем 27421 id, а сужение по виду — потом, фильтром через
`sites()` по двум десяткам финалистов. Обратный индекс «вид → id» к тому времени уже существовал,
но его спрашивал только автокомплит. Разница в цене: `entity` — 14274 id вместо 27421, `graph` —
96, `categories` — 8. Дальше те же дешёвые фильтры, что у ключей локализации, но общий префикс
всего два символа: id пишутся PascalCase и опечатка бывает в любом месте слова, тогда как ключ
локализации несёт тему в начале. Настоящий id при этом из-под фильтров не выпадает никогда —
у него та же длина и то же начало, что у опечатки, — так что на полноту подсказок фильтры не
влияют вовсе, только на цену.

Второе — память на проход. Подсказки строятся эагерно, в момент создания `IdProblem`, то есть
на каждом проходе демона и **на каждую ссылку**: на файле, где один сломанный базовый id стоит
под многими `parent:`, в логе видно 17 одинаковых вызовов подряд, по 26 мс каждый — полсекунды
чистого Левенштейна на проход, ради трёх одних и тех же имён. Ответы мемоизируются
`ConcurrentHashMap` в `CachedValuesManager` с зависимостью от `PsiModificationTracker.MODIFICATION_COUNT`:
трекер тикает на любую правку, поэтому мапа живёт ровно один проход демона и не растёт вслед за
набираемым текстом. Замер после правки: 71 опечатка на 14274 id вида `entity`, 69 подсказаны
обратно (две теряются из-за `take(3)`, а не из-за фильтров), среднее 2 мс на вызов.
...
**`FlagSerializer` резолвится обратным поиском, а не по имени тега.** Трюк `ConstantSerializer`
здесь не работает: из шести тегов на своё имя похож один — `AtmosDirectionFlags` → `AtmosDirection`,
тогда как `CollisionLayer` и `CollisionMask` оба ведут в `CollisionGroup`, а `VisibilityMaskLayer` —
в `VisibilityFlags`. Карту строит сам движок (`SerializationManager.FlagsAndConstants`): каждый
`[Flags]`-enum регистрируется под каждым своим `[FlagsFor(typeof(tag))]`, второй enum на тот же тег —
исключение, то есть отношение всегда однозначно. Поэтому хост ищет **тег**
(`Finder.FindReferences(tag, domain, NullProgressIndicator.Create())`, домен — всё решение по той же
причине, по которой константы ищутся в `LibrarySymbolScope.FULL`: теги лежат в `Robust.Shared`, enum'ы
в `Content.Shared`), от ссылки поднимается до `IAttribute`, проверяет имя `FlagsFor` и берёт члены
enum'а с носителя атрибута. Ссылок у тега единицы: сам атрибут плюс поля, которыми он сериализуется.
Enum из скомпилированной сборки деклараций не имеет и не найдётся — тогда список пуст, а пустой
список означает «здесь ничего не известно», то есть молчание вместо ошибки.

**Значения кастомного сериализатора сравниваются с учётом регистра, а обычного enum — без.**
`SerializationManager.ReadEnumValue` — это `Enum.Parse(value, true)`, и контент пишет `Belt` рядом
с `BELT`. А `ConstantSerializer.Read` и `FlagSerializer.Read` зовут `Enum.Parse` **без** флага, то
есть `belowMobs` там ошибка загрузки. Различать их отдельным полем в модели не пришлось: оба
источника таких значений — это ровно `customSerializer`. На чекауте это не стоит ничего: все 398
значений `drawdepth` и все значения флагов написаны так же, как объявлены.

**Флаги нельзя проверять по имени ключа, хотя полей всего пять.** `layer`, `mask`, `visMask`,
`airBlockedDirection` — казалось бы, зашей и работай без солюшена. Но имя ключа глобально, а тип
принадлежит владельцу: `mask:` у `PointLightComponent` — путь к текстуре
(`/Textures/Effects/LightMasks/cone.png`), у одежды — id прототипа (`ClothingMaskBreath`), а `layer:`
у слоя спрайта — `enum.HumanoidVisualLayers.Chest`. Правило по имени покрасило бы 122 рабочих
значения (`MeasureFlags`); часть остального спасает только то, что проверка молчит на всём, что не
идентификатор. Одна скалярная запись при этом несёт несколько имён — `Enum.Parse` сам режет по
запятой, — поэтому списочная форма и `mask: Impassable, Opaque` разбираются одинаково.
...
**Мёртвый ключ локализации опознаётся по соседям, когда тип молчит.** `wordReplacements` объявлен
`Dictionary<string, string>`, и что обе его стороны — ключи локализации, знает только тот, кто поле
читает: `ReplacementAccentSystem` отдаёт каждую в `Loc.GetString`. Протащить это от `[DataField]`
до аргумента вызова — анализ потока данных, а локальность отвечает на тот же вопрос даром: где
в маппинге лежит толпа ключей и все, кроме одного, объявлены, одинокий промах — опечатка, а не
значение другого рода. Порог мягкий (два живых соседа и половина маппинга) и работу делает не он:
ответ перестаёт меняться далеко ниже самого слабого проверенного условия.

Работу делают два отбора. Ключ, который означает id (`id`, `parent`, `proto`, `prototype`, `entity`),
и значение, объявленное id прототипа: без второго `flavor: raw-egg` читается мёртвым сообщением,
потому что вкусы в `Flavors/flavors.yml` пишутся тем же kebab-case. Поле, которое бэкенд назвал
`LocId`, пропускается целиком — его проверяет `localizationIds`, и два предупреждения на одном
значении не нужны.

**Форма ключа тут строже, чем у `looksLikeMessageId`.** `metamorphicFillBaseName: fill-` — это
префикс имени состояния спрайта, он стоит рядом с живыми `reagent-name-*`, и висящий дефис
`looksLikeMessageId` пропускает: там форма мягкая намеренно, потому что у поля типа `LocId` автор
волен написать что угодно. Здесь же текст — единственная улика, поэтому требуется тот же регекс,
что у индексов использований (`[A-Za-z][A-Za-z0-9_]*(?:-[A-Za-z0-9_]+)+`). Цена ошибки — 150
находок в файлах реагентов, и прикидка на python её не показала: её собственный regex случайно был
строже отгружаемого кода. Поймал это `MeasureSiblings`, потому что зовёт `deadSiblings`, а не
пересказ правила.

Замер по ss14-wega: 24 находки в 7 файлах, ложных ноль. Семь из них не ловит больше ничто —
`JobPrototype.Description` объявлен `public string?`, та же ловушка, что у `AntagPrototype.Name`,
поэтому типизированная проверка на него не смотрит вовсе. Правило читается с маппинга целиком, а не
с каждого ключа: соседи одни и те же, и обход на каждый ключ был бы квадратичным на
`wordReplacements`, где под одним ключом лежат сотни записей. По той же причине объявленность
спрашивается у кэшированного отсортированного списка (`declaresMessage`, `declaresId`), а не у
индекса запросом на каждое значение.
...
**Гайдбука не было ни в одном корне, и замер этого показать не мог.** `Resources/ServerInfo` не
кончается на `Prototypes` и не называется `Locale`, то есть не попадал в `indexedRoots` — а
`FileBasedIndex` читает только content roots и корни библиотек. Значит `RobustLocaleUsageIndex`,
который с самого начала разбирает `<FTLTextpart Key="…"/>`, в IDE этих файлов не видел вовсе:
476 ключей из 495 не используются больше нигде — ни из `.cs`, ни из YAML, ни placeable'ом из другого
сообщения, — и `RobustUnusedLocalizationAnnotator` красил их все как мёртвые. Замер этого поймать не
мог по построению: он читает диск напрямую и про indexable file sets ничего не знает. Тот же корень
чинит и запись: `RobustFileWritingAccess` выдаёт разрешение по `indexedRoots`, а без него Shift+F6
в гайдбуке останавливается до диалога ввода — файл библиотеки платформа считает не-проектным.

**Ссылки гайдбука — обычный PSI, а не диапазоны руками.** У XML на фронтенде есть парсер, у
`XmlAttributeValue` — манипулятор (`lang.elementManipulator` в `intellij.xml.psi.impl.xml`), а в
`backend.actions.support` регистрации для XML закомментированы, то есть все три действия остаются
фронтенду. Поэтому `PrototypeIdReference` перестал быть `PsiReferenceBase<YAMLScalar>` и принимает
любой элемент: Ctrl+клик, Alt+F7 и Shift+F6 из `<GuideEntityEmbed Entity="Crowbar"/>` работают тем же
кодом, что и в YAML, без `FakePsiElement` и правок через документ — в отличие от `.ftl` и `.cs`.
Обход в `processReferences` для гайдбука идёт по `XmlAttributeValue`, а не по `YAMLScalar`;
сообщений там не ищут — `Key="…"` покрыт индексом использований локализации.

**Имя тега — часть шаблона, иначе индекс соберёт весь чекаут.** `Entity="…"` — обычный атрибут, и
одного его мало: ссылкой считается только атрибут на теге `Guide*`. Ссылочных атрибутов три —
`Entity` (1193 вхождения, вид `entity`), `Reagent` (42, `reagent`) и `Discipline` (5,
`techDiscipline`); `Caption` — текст для читателя, а `Group` у `GuideReagentGroupEmbed` и
`GuideMicrowaveGroupEmbed` сравнивается со строковым полем прототипа, а не с его id. Проверять эти
id стоит ошибкой: `GuideEntityEmbed.TryParseTag` идёт прямо в `IEntityManager.SpawnEntity`, а
реагентный и дисциплинарный embed'ы отказываются собирать тег. В контенте мёртвая ссылка одна —
`MachineTraversalDistorter` в `Science/TraversalDistorter.xml`, и `migration.yml` подтверждает
удаление (`MachineTraversalDistorter: null`).

**Атрибуты читаются по одному, а не одним regex от головы тега.** Форма
`<Guide\w*\s[^>]*?Entity="([A-Za-z][\w.]*)"` роняет три вещи, которые XML разрешает: одинарные
кавычки, второй ссылочный атрибут того же тега (поиск продолжается за первым совпадением) и `>`
внутри значения соседнего атрибута (`Caption="a > b"` обрывает `[^>]*?`). Цена промаха
односторонняя и оттого незаметная: PSI такой атрибут читает, Ctrl+клик работает, а файл просто не
попадает в кандидаты — Find Usages его не видит, rename проходит мимо. Поэтому текстовое правило
устроено как у PSI: `<Guide\w*` находит голову тега, дальше `matchAt` читает атрибуты подряд от
конца предыдущего (`\s+имя\s*=\s*("[^"]*"|'[^']*')`), и обход сам кончается на `>` или `/>`.
В ss14-wega ни одной из трёх форм нет — это и есть причина чинить сразу: замерять потом будет
нечего, а `MeasureGuidebook` слеп по построению, он зовёт ту же функцию.
...
**Ссылкой считаются пять имён и всё, что индекс знает как `ProtoId`, — но спрашивать надо индекс,
а не бэкенд.** `parent`/`proto`/`prototype`/`entity`/`id` были единственными ссылками, поэтому
`prototypes:` у спавнера, `result:` у рецепта и `graph:` у графа конструкции не имели ни Ctrl+клика,
ни Find Usages; хуже того, rename переписывал объявление и пятёрку имён, а 23879 значений контента
оставались со старым id — молча, до первой типизированной проверки на прогретом бэкенде.

Взять вид у бэкенда здесь нельзя, и дело не в скорости. `cachedField` знает только те типы, которые
уже спрашивали: файл под кареткой прогрет аннотатором, а те сотни файлов, по которым идёт rename, —
нет. Значит один и тот же rename переписывал бы разный набор файлов в зависимости от того, что успел
прогреться, — недетерминированная правка контента хуже честно неполной. Поэтому вид спрашивается
у regex-индекса: `[DataField]` объявлен `ProtoId<T>`, `EntProtoId` или через сериализатор — это
читается из текста `.cs` и одинаково доступно для любого файла, открытого или нет.

Правило слабее бэкендовского: имя ключа глобально, а тип принадлежит владельцу. Цена ошибки при
этом разная у трёх потребителей — Ctrl+клик промахнётся, Find Usages покажет лишнюю строку, а
rename **пишет**, то есть ложная ссылка означает переписанное чужое значение. Отсюда отбор: имя
принимается, только если **ничто** в чекауте не объявляет его иначе, и обе половины лежат в индексе
отдельными ключами (`protoField:` и `plainField:`), чтобы ответ считался разностью двух перечислений
ключей — без чтения значений. Замер `MeasureTypedIds`: 922 имени объявлены `ProtoId` хоть где-то,
110 из них где-то ещё обычные поля (`name`, `values`, `tags`, `key`, `layer`) — без этой разности
под правило попадает 85244 значения, которые вообще не id, включая 249 совпадающих с настоящими
id и потому переписываемых. Оставшиеся 812 имён несут 23879 значений, и все до одного объявлены
прототипами в контенте.

**У словаря id лежат в значениях маппинга, и имя над ними не знает ничего.** `equipment:` у
`ChameleonOutfitPrototype` объявлен `Dictionary<string, EntProtoId>`: ключи там — имена слотов,
их придумывает автор, а id стоит справа. Правило по имени ключа такое значение не видит вовсе —
над `ClothingMaskBreath` написано `mask`, а это датафилд ничего, да ещё и одно из 110 неоднозначных.
Ctrl+клик отвечал «Cannot find declaration to go to», Find Usages проходил мимо, rename не правил.
Поэтому у индекса появился третий ключ, `protoValueField:`, и вопрос задаётся значению, а не имени:
ссылка — это либо своё имя ключа, либо имя ключа над маппингом, в котором значение лежит. Подъём
ровно на один уровень: `mask:` под `equipment:` — та форма, которой пишет контент, а дальше имя
за два вложения начало бы забирать значения, про которые ничего не знает.

Записывается только сторона значений. `Dictionary<ProtoId<X>, int> Materials` держит id в ключах
маппинга, а ключ — не то место, где сейчас может жить наша ссылка; принять его за сторону значений
значило бы пустить rename по количествам справа. Второй ключ не заменяет первого: имя попадает
в оба, когда тип говорит и то и другое, — они отвечают про разные места в YAML. Из семи
сериализаторов сторону значений задаёт ровно один (`PrototypeIdValueDictionarySerializer`), его
близнец по ключам отличается одним словом в имени.

Замер `MeasureTypedIds` (вторая половина): 29 имён объявлены словарём с id в значениях, 5 отвергнуты
как неоднозначные, под оставшимися 24 лежит 4072 значения, и все до одного объявлены прототипами
в контенте — `NOT AN ID IN A MAPPING: 0`.

**У маппинга две половины, а проверялась одна.** `prototypeIdValues` читал у значения-маппинга
только ключи (`keyPrototypeKind` — это `Dictionary<ProtoId<X>, int>`), поэтому
`cash: { 1: Telecrysta }` не проверялся ничем: id стоит справа, а `prototypeKind` у словарного поля
как раз и есть вид **значения** (хост считает его тем же `UnwrapType`, что и для коллекций).
Спрашивать надо ключ словаря целиком — `cash:` это обычный датафилд объявления, и бэкенд отвечает
по нему без всякого спуска, — а потом пройти по значениям его записей. Отдельного правила по индексу
для этого не нужно: вид приходит с типом, значит работают и сообщение, и quick fix, и уровень.

Тем же одним уровнем чинится автокомплит: `expectedKind` у значения внутри словаря спрашивает поле
над маппингом и берёт его `prototypeKind`, но только если у поля взведён флаг `dictionary`. Флаг тут
не украшение — под вложенной структурой ключи это настоящие поля, и наследовать им вид владельца
значило бы предлагать id там, где стоит что угодно другое.

**Новый ключ индекса не появится, пока не поднят `getVersion`.** Данные `FileBasedIndex` лежат
на диске и пересчитываются только по версии индексера, поэтому `protoValueField:`, добавленный
при `getVersion() = 8`, в песочнице не существовал вовсе: правило читало старый индекс, получало
пустое множество имён и молчало — ни ошибки, ни лога, потому что чекаут без словарей выглядел бы
точно так же. Проверка руками показала ровно то же «Cannot find declaration to go to», что и до
правки, и списать это на что угодно другое было легко. Правило простое: меняется состав того, что
пишется в индекс, — поднимается версия.
...
лишний ключ стоит лишнего файла, но не ложного результата. Из `.cs` он берёт только id
(`RobustCodeLinks.ids`): пути оттуда никто не ищет, а ключи локализации уже лежат в своём индексе. Замер `MeasureUsages`: 3100 файлов за
...
**Интерполяция ломала сканер литералов, и это был баг rename, а не инспекции.**
`$"\n{Loc.GetString("ban-list-unbanned-by", x)}"` — автомат видел `$` кодом, `"` открывал строку,
а кавычка перед ключом её **закрывала**: сам ключ становился кодом, в индекс не попадал, и Shift+F6
его не переписывал. Таких ключей 43. Сторож `MeasureLocaleRename` этого не ловил по построению — он
проверяет только «сканер не нашёл больше наивного regex», а тут сканер нашёл меньше. Дырка кода —
это код, поэтому у состояний появился стек: `{` внутри строки кладёт состояние и глубину скобок,
`}` на нулевой глубине возвращает. `{{` и `}}` — литеральная скобка, вложенная интерполяция
возможна, а `$"""` (сырая интерполированная, C# 11) раньше уходила в состояние RAW целиком —
три ключа в `BanDef.cs` были видны только оттуда.

**Ключ живёт ещё в двух видах файлов, и оба не про C#.** XAML пишет `Text="{Loc 'ui-options-title'}"`
— 1560 ключей в 305 файлах, причём в двух формах (1326 в кавычках, 378 без) и иногда с неймспейсом
(`{controls:Loc '...'}`, 7 ключей). Гайдбук — это XML в `Resources/ServerInfo`, где ключ стоит
атрибутом `<FTLTextpart Key="..."/>`, 495 штук; брать любую строку XML нельзя, иначе приезжает
`encoding="utf-8"`. Оба источника теперь в `RobustLocaleUsageIndex`, то есть их видят и Find Usages,
и rename. Там же нашлось, что гайдбук ссылается и на прототипы (`GuideEntityEmbed Entity=`, 1194
значения) — это отдельная дыра, уже не в локализации.

**Форма ключа в `RobustYamlValueIndex` была строчной, и это теряло 330 использований.** Ключи несут
имена: `job-description-wardenHelper`, `contraband-examine-text-GrandTheft`,
`tts-voice-name-Cyberpunk_dexter`. Регекс `[a-z][a-z0-9]*(?:-[a-z0-9]+)+` их не признавал, файл не
попадал в кандидаты, и ключ выглядел неиспользуемым. Расширение до `[A-Za-z][A-Za-z0-9_]*` делает
индекс чуть больше, а ошибиться им нельзя: лишний ключ стоит лишнего файла, но не ложного ответа.

**Ссылкой на сообщение считается не только поле типа `LocId`.** `AntagPrototype.Name` объявлен
`string`, а `Loc.GetString` зовётся уже на месте использования, поэтому по типу поля
`name: roles-antag-heretic-name` ссылкой не является вовсе: Ctrl+клик молчал, а Alt+F7 отвечал двумя
объявлениями и ни одним использованием. Замер: значений-сообщений в прототипах 12040 на 9572 разных
сообщениях, тогда как через типизированный `LocId` их 2668 — то есть три четверти ссылок поиск не
видел. Признак взят тот же, что у ховера: дефис в значении плюс объявленное сообщение в индексе,
чего не бывает ни у id прототипа, ни у имени компонента. Валидация при этом **не** расширяется —
ругаться на неизвестный id там, где поле обычная строка, значит красить половину контента; поэтому
`RobustValidation` по-прежнему смотрит `field.localized`, а расширен только контрибьютор ссылок,
и через него — Ctrl+клик, Find Usages и rename.

**Ключ собирают тремя способами, и третий нашёлся только на живом файле.** Начало (`"cmd-" + command`,
`$"accent-{name}-replacement"`) и `localizedDataset` с префиксом и счётчиком были известны сразу.
Третий — вывод одного ключа из другого: `node.Description.Replace("-desc", "-tooltip")` в
`HereticStoreMenu`, где целиком ключ не написан нигде, а в коде лежат только литералы `"-desc"` и
`"-tooltip"`. Проверка объявила мёртвыми два рабочих тултипа. Начало кончается дефисом, конец с
дефиса начинается, поэтому оба живут в одном индексе и различаются формой сами. Хвосты берутся
только из файла, который зовёт `GetString`: без этого условия приезжают `"-c"`, `"-o"`, `"-unshaded"`
и глушат целые семейства ключей. Цена учёта хвостов — 26 замолчавших из 1776; восемь литералов на
весь чекаут (`-desc`, `-tooltip`, `-all`, `-nonmob`, `-watch`, `-mac`, `-win`, `-linux`).

**Сторож не ловит собранные ключи по построению.** `SPELLED OUT SOMEWHERE: 0` проверяет, что ключ
нигде не написан, а собранный ключ и не должен быть написан — именно поэтому тултипы прошли сторож
и всплыли только у пользователя. Единственная защита здесь — держать список аффиксов широким и
помнить, что ложное срабатывание дороже пропуска.

**Мёртвое сообщение показывается аннотатором по языку `TEXT`.** Инспекция здесь не работает по той
же причине, что и на прототипах: `Locale` подключён `SyntheticLibrary`, а
`HighlightingSettingsPerFile.shouldInspect` для файла из library scope вне content возвращает false.
Plain text этому не мешает — платформа сама вешает на него аннотаторы (`LargeFilesAnnotator` в
`LangExtensions.xml`, `CommitAnnotator` у Grazie), и по байткоду `LargeFilesAnnotator.annotate`
начинается с `element instanceof PsiFile`: на файле без парсера элемент один, и это сам файл.
`DefaultHighlightVisitor.suitableForFile` возвращает `true` безусловно, то есть аннотаторы
запускаются на любом файле. Отключается галочкой в настройках — у аннотатора, в отличие от
инспекции, своего выключателя нет.
...
**В `.cs` фронтенду достаётся ровно одно действие из трёх, и решает это не EP, а политика.**
`CSharpActionSupportPolicyBase.getCallStrategy` (EP `backend.actions.support`, `language="C#"`)
отвечает `BACKEND_FIRST` на `GotoDeclaration`, `FRONTEND_FIRST` на `FindUsages` — но **только если
элемент оказался `PsiFile`**, — и `BACKEND_ONLY` на всё остальное, включая `RenameElement`.

Ctrl+клик работает потому, что `BACKEND_FIRST` — это `isFrontendInvolved = true` (флаги enum
`ActionCallStrategy`: `BackendOnly(false, true)`, `BackendFirst(true, true)`): ReSharper отвечает
первым, мы запасной вариант, а на строковом литерале объявлять ему нечего.

Alt+F7 не работает: каретка в открытом файле никогда не даёт `PsiFile`. Парсер C# на фронтенде
(`CSharpKotoParserDefinition`) строит настоящее дерево, поэтому `ActionCallPolicy.getElement`
возвращает `psiFile.findElementAt(offset)` — узел, а не файл. Своих `usageTargetProvider` и
`findUsagesHandlerFactory` для C# Rider не регистрирует вовсе.

Shift+F6 не работает, и запись об обратном тут стояла по ошибке — стоит объяснить, в чём был подвох.
`FrontendRenameHandlerRegistry` (подменяет `RenameHandlerRegistry`, `intellij.rd.client.jar`) в
`getRenameHandler` и `hasAvailableHandler` действительно **сначала** прогоняет весь EP `renameHandler`
и отдаёт первое расширение с истинным `isRenaming` (по умолчанию это `isAvailableOnDataContext`),
и только потом смотрит на стратегию. Но этот цикл живёт внутри платформенного `RenameElementAction`,
а оно в `.cs` не запускается: действие подменено на `BackendDelegatingAction`, чей `actionPerformed`
идёт в `performBackendDelegatingAction`, там читает ту же стратегию и при `BACKEND_ONLY` зовёт
`executeBackend` → `backendActionPerformed`. Подмена происходит уровнем выше любого EP платформы,
поэтому «отобрать» действие расширением нельзя в принципе.

Перебить это можно только своим `backend.actions.support` для C# с `order="first"` — то есть заменив
маршрутизацию **всех** действий C# в Rider на свою. Цена не стоит ключа: получить оригинальную
политику неоткуда, пришлось бы переписать её три правила у себя, и любое их изменение в новой версии
ломало бы у пользователя рефакторинги C# целиком.

Поэтому со стороны `.cs` остаётся Ctrl+клик, и он же мост: перешёл в `.ftl` — там и Alt+F7, и
Shift+F6 работают, а литералы `.cs` в результатах и правках учитываются с самого начала.

**Тот же мост построен для id прототипов, путей ресурсов и состояний спрайта.** Из `.cs` работает
Ctrl+клик (`RobustCodeDeclarationHandler`), обратно — Alt+F7 и Shift+F6 с YAML-стороны, потому что
литералы `.cs` теперь попадают в `RobustYamlValueIndex` и в обход `processReferences`. Половина
«в C#» тут не украшение: до неё переименование id в редакторе оставляло `EntProtoId Mob = "MobHuman"`
смотреть в пустоту — молча, потому что строковый литерал продолжает компилироваться и промах виден
только в игре. Замер `MeasureCodeLinks` по ss14-wega: 1127 литералов-id (783 разных), 1133 пути
(705 разных, 972 резолвятся) и 62 состояния.

**Улика у id — тип рядом, и её хватает только на одной строке.** `"MobHuman"` от `"Sprite"`
неотличим, поэтому литерал считается id, лишь когда перед ним на той же строке написан `EntProtoId`
или `ProtoId<X>`; из 1123 объявлений контента так записаны 1117. Шесть оставшихся — элементы списка
под `= new() {`, и ради них правило смотрит на одну строку выше, но только если та и называет тип,
и кончается открытым `=`, `{` или `[`. Дефис в значении отбрасывает литерал целиком: это ключ
локализации, и он принадлежит `RobustLocaleUsageIndex` — два правила не пересекаются по построению.

**Путь опознаётся ведущим `/`, а отсеивается существованием файла.** Индекс про соседние файлы
ничего не знает, поэтому в тексте проверяется только форма; резолв делает `RobustResources`, и
литерал, за которым файла нет, просто не даёт перехода. Цена нулевая: пути в индекс не пишутся
вовсе — из YAML их никто не ищет, — а «мимо» бывает часто и законно. Из 1133 путей 128 вообще не
про ресурсы (`/admin/info` и прочие маршруты серверного API), ещё 33 указывают под `Resources` на
то, чего там нет: собираемые префиксы (`/Fonts/NotoSans/NotoSansSymbols-`), шаблоны с `{2}` и
фикстуры тестов движка.

**Ссылка на литерале лучше хендлера по offset, и с 2026.2 она возможна.** `RobustCodeLinks`
по-прежнему решает, что литерал называет — правило текстовое, потому что тем же правилом живёт
индекс, у которого PSI нет вовсе, — но носителем стал сам `CSharpStringLiteralExpression`. Даёт это
три вещи разом: подчёркивание под Ctrl, честный `rangeInElement` вместо «где-то в файле» и точку,
за которую цепляется автокомплит. Диапазон берётся `ElementManipulators.getValueTextRange(element)`,
а не `getContentRange()`: манипулятор зарегистрирован платформой, и через него форма литерала
(обычный, verbatim, raw, интерполированный) плагину знать не нужно.

Старый `gotoDeclarationHandler` оставлен и уступает дорогу сам: если у литерала под кареткой уже
есть наша ссылка, он молчит, иначе дублировал бы цель в попапе. Проверка сделана вопросом к
элементу, а не предположением, — если механизм ссылок однажды перестанет работать, переход
продолжит жить на хендлере.

**Автокомплиту нужен подрезанный `DUMMY_IDENTIFIER`.** Платформа пишет в копию файла
`IntellijIdeaRulezzz ` — с пробелом на конце, — а все три правила («это id», «это состояние», «это
путь») пробел отвергают: ни одно из этих значений с пробелом не пишется. С пробелом в копии ссылки
нет вовсе, и на пустом литерале — там, где автокомплит и зовут, — список пуст. Поэтому
`RobustCodeCompletionContributor` не строит ни одного элемента, а только переставляет
`context.dummyIdentifier` на `DUMMY_IDENTIFIER_TRIMMED`; сами элементы приезжают из `getVariants`
наших ссылок через платформенный `LegacyCompletionContributor` (`language="any"`, `order="last"`).
Что контрибьюторы платформы в C# вообще работают, видно по политике автопопапа Rider —
`BackendAndGenericIJ`, то есть бэкенд **и** обычный IJ.

**Ховер в `.cs` держится на том, что Rider уступает.** `RiderDocumentationTargetProvider` подменяет
сервис `IdeDocumentationTargetProvider`, но в `documentationTargets(editor, file, offset)` при
неудаче зовёт `IdeDocumentationTargetProviderImpl` — то есть обычный список EP
`platform.backend.documentation.targetProvider`. Неудача — это `null` из
`tryCreateDocumentationTarget`, а он отдаёт `null`, когда бэкенд не вернул сессию; на строковом
литерале ReSharper'у документировать нечего. Проверено в песочнице на всех четырёх видах значений;
там же видно, что наш таргет выигрывает у карточки файла, которую даёт резолвнутая файловая ссылка:
на пути к `job_icons.rsi` показывается сетка из 79 состояний, а не сведения о каталоге. Карточка
платформы остаётся там, где нам сказать нечего, — например на `.ogg`. Поэтому
`RobustCodeDocumentationProvider` переиспользует
те же таргеты, что и YAML (`RsiContentsTarget`, `SpritePreviewTarget`, `PrototypeDocumentationTarget`,
`MessageDocumentationTarget`) — их пришлось из `private` сделать `internal`, и это вся правка
на стороне YAML. Вид прототипа тут не спрашивается у бэкенда, как в YAML: `ProtoId<X>` называет его
прямо, и достаточно сузить объявленные виды до него — тогда таргет читает id как единственное
объявление (`untyped = true`, `kinds.singleOrNull()`).

**Состояние спрайта в C# опознаётся соседним аргументом, а не своей формой.** У
`new SpriteSpecifier.Rsi(new ResPath("/Textures/Interface/Misc/job_icons.rsi"), "Syndicate")` второй
литерал — кадр внутри `.rsi`, и сказать это может только путь, стоящий перед ним: сам по себе
`Syndicate` неотличим от id прототипа, которым он к тому же и является (антаг, департамент и ещё
двое). Правило поэтому смотрит на **предыдущий** литерал и на промежуток между ними — там
допустимы только кавычки, скобки, запятая и пробелы, — а не на строку: из 62 таких мест в 13 файлах
состояние часто написано на следующей строке. Форма самого значения нарочно свободная, с дефисом
(`equipped-INNERCLOTHING`): что это состояние, уже сказал путь, а кадра либо нет в каталоге, либо
переход состоится. Проверка ветки обязана идти **до** проверки на id, иначе дефисные состояния
не опознаются вовсе. Замер: 62 состояния, ни одного без кадра.

**Вид из `ProtoId<X>` — это то, чем правило для rename строже правила для перехода.** Промах
Ctrl+клика стоит бездействия, а промах rename переписывает строку в чужом коде, который после этого
компилируется и падает только в игре. Поэтому ссылка строится, лишь когда id объявлен **под тем
видом**, за который стоит класс `X` (`EntProtoId` — всегда `entity`: параметр его дженерика
ограничивает компонент, а не вид). Класс переводится в вид инверсией `RobustDataFields.prototypeClass`:
обратного индекса нет и не нужно — видов 205, карта кэшируется рядом с ними. Сработало это сразу:
`ProtoId<NpcFactionPrototype> RevPrototypeId = "Rev"` называет фракцию, которой нет (в контенте она
`Revolutionary`), тогда как антаг `Rev` есть, — без проверки вида переименование антага переписало
бы эту строку. Когда вид неизвестен (класс не прототип, `ProtoId<T>` в дженерик-коде), решает один
id, как и везде; на ss14-wega таких классов ноль, и это сторож замера.

**Ключ под кареткой в `.cs` ищется тем же сканером, что и индекс, и обязан быть объявленным.**
Читать строку вокруг каретки нельзя: `// see "comp-thief-target"` — это проза, и Shift+F6 предложил бы
переименовать по комментарию. А проверка `hasMessage` нужна с другой стороны: дефис в литерале ещё
ничего не значит (`"utf-8"`, `"multipart-form-data"`), и без неё рефакторинг отбирался бы у ReSharper
на каждой такой строке. Результат сканера кэшируется на файл: оба вызывающих спрашиваются на каждое
нажатие клавиши — `gotoDeclarationHandler` через `CtrlMouseHandler`, а доступность Shift+F6
пересчитывается на каждом update действия. Update — не read action, поэтому его приходится брать
самим, плюс выход в dumb mode: индекс во время индексации не ответит.
...
**Проверка `state:` держится на трёх правилах движка, и два из них — про молчание.** Спрайт ищется
в порядке `SpriteComponent`: свой `sprite:` у слоя, затем `sprite:` компонента, затем тот же
компонент у предков — накопительно и до конца цепочки, потому что `ComponentRegistrySerializer`
мержит компоненты перед чтением, а базы вроде `BaseItem` абстрактны. Молчим в двух случаях, и оба
взяты из кода, а не из осторожности. Первый: `if (layerDatums.Count == 0)` — верхнеуровневое
`state:` превращается в слой, только когда слоёв нет, поэтому рядом с `layers:` оно не читается
вовсе (149 значений в чекауте, и в `jobs.yml` это выглядело сломанным кадром). Второй:
`abstract: true` — такой прототип не инстанцируется, и спрайт ему может дать потомок, а не предок
(`BenchBaseMiddle` объявляет `state: middle` и оставляет `sprite:` скамейкам ниже); ровно поэтому
же абстрактные обходит `RobustRequiredFields`.

Проверка на `layers:` обязана стоять **до** обращения к `sprite:` своего маппинга: у компонентного
`state:` владелец — сам компонент, и его `sprite:` возвращался первым, делая правило недостижимым.
Замер этого не видел (он спрашивает `owner == component` до разрешения), поймал тест.

**Уровень — ошибка, и обоснование двойное.** `SpriteComponent` пишет
`Logger.ErrorS("State '{0}' does not exist in RSI")` и подставляет fallback-текстуру, то есть спрайт
сломан при каждом спавне, а не «когда-нибудь при использовании» — этим случай отличается от мёртвого
id в поле, оставшегося предупреждением. Плюс `ClientSpriteSpecifierSerializer.ValidateRsi` отдаёт
`ErrorNode("Invalid RSI state")`, то есть для формы `{ sprite:, state: }` мёртвое состояние роняет
`Content.YAMLLinter` в CI.

`MeasureStates` по ss14-wega: 30585 объявлений (1299 абстрактных пропущено), 13315 значений `state:`
внутри компонентов, 11520 проверено (5507 — через унаследованный спрайт), молчим на 149 «рядом с
layers», 1449 «спрайт не выводится» и 197 «`.rsi` нет в чекауте». Находок три, все настоящие:
`corgi_eyes_displacement` дважды (в каталоге есть `corgi_head`, `corgi_belt`, `corgi_mask`, но не
`eyes`) и `equipped-HELMET` в `teg.rsi`, где лежит один `icon.png`. Ложных ноль, полный проход
540 мс.

**Замер прототипов разбирает YAML деревом, а не построчными регексами.** `tools/measure/Block.java` —
минимальный блочный парсер: маппинги, последовательности, скаляры, дефис с первым ключом своего
элемента. Причина в том, что вопрос «какому маппингу принадлежит этот ключ» плагин решает по PSI, а
замер по колонкам — и расходятся они там, где контент необычен. Первая же версия парсера отдавала
элементы последовательности не тому владельцу (SS14 пишет `- type:` на уровне самого `components:`,
и правило «выталкивать при `<=`» уводило элемент к объявлению): вместо 14000 значений `state:`
нашлось 949, и в выводе об этом не было ни слова. Отсюда и сторож замера — не находка, а порог:
меньше 10000 проверенных значит, что обход ослеп.

**Один id — не один прототип, и всё, что спрашивает индекс по id без вида, врёт.** `Binoculars` —
это и сущность, и `latheRecipe`, который её собирает; таких совпадений в контенте много. Инлей над
`id:` у рецепта показывал имя предмета, а Ctrl+H по нему уводил в предков сущности (`BaseItem`),
которых у рецепта нет вовсе. Вид берётся из **собственного** `- type:` объявления
(`RobustYamlContext.declarationAround`), а не из `sites(id).first()`; в иерархии он же передаётся в
`findDeclarations` и отсекает чужие ветки при обходе вниз — наследование внутри вида и между видами
не ходит. Поймано вопросом «как рецепт наследует BaseItem», а не проверкой: замеры считают контент,
а тут неверен был сам вопрос к индексу.

**В подсказке разметка снимается, а не печатается.** Имена несут rich text (`[color=purple]Укус
[/color]`), и инлей показывал его буквально — притом что игрок видит слово, а не теги. Раскрасить
нельзя: у декларативной подсказки один `HintFormat` на всю строку, цвета по фрагментам там нет.
Поэтому `RobustMarkup.plain` — тот же разбор, что и `toHtml`, но без HTML: зарегистрированные теги
выбрасываются, всё прочее остаётся текстом, и правило про `[redacted]` действует ровно так же.

**Иконки в автокомплите рисует рендерер, а не `withIcon`.** Список id вида `entity` — это 14274
элемента, а `LookupElementRenderer` платформа зовёт только для видимых строк, то есть для десятка.
С `withIcon` пришлось бы пройти цепочку предков и прочитать PNG четырнадцать тысяч раз, чтобы
показать двенадцать. Кадр берётся тот же, что в гуттере, и тем же размером: строка списка высотой
в строку, крупнее всё равно ужмётся.

**Ctrl+H строится на графе id, а не типов.** Узел дерева — объявление id, найденное индексом, рёбра
читаются из `parent:` в обе стороны. Вниз отдельного индекса не понадобилось:
`RobustYamlReferenceIndex` с самого начала пишет каждый id, названный файлом в `parent:`, под своим
префиксом — значит файлы-кандидаты берутся одним запросом, и открываются только они. Текст узла
задаётся через `myHighlightedText`, потому что `PsiNamedElement` у `YAMLKeyValue` — это ключ, то
есть слово `id`. `TypeHierarchyBrowserBase` требует девять методов от себя и от
`HierarchyBrowserBaseEx`; из них содержательны два — `createHierarchyTreeStructure` и
`isApplicableElement`, остальное подписи и заголовки. Две подписи всё же пришлось задать: без них
вкладка называется «Class id» — платформенное слово для иерархии типов плюс `PsiNamedElement`
у `YAMLKeyValue`, а имя у него это ключ, буквально `id`. `getContentDisplayName` печатает
`Binoculars (latheRecipe)`, а `getPresentableNameMap` переименовывает три вида в Prototype,
Children и Parents: классов здесь нет вовсе.
...
попавшимся `getParentOfType`. Такие вещи закрываются тестами: `./gradlew :test`, 220 случаев,
5 секунд.
...
разбор `migration.yml` и достижимость целей quick fix, а заодно гоняет сам фикс опечатками
(`ChangePrototypeIdFix.suggest` по id самого широкого вида — цена вызова и доля подсказанных
обратно), `MeasureRequired` — инспекцию обязательных полей. `MeasureLocaleRename` — второй сторож с ненулевым кодом возврата: он сверяет посимвольный
...
`MeasureSiblings` — шестой: он прогоняет `RobustValidation.deadSiblings` по каждому маппингу
прототипов и падает на `FALSE POSITIVES` — ключе, который объявлен в чекауте чем-то другим
(id прототипа, имя компонента или всё-таки сообщение). Именно этот сторож поймал `fill-`.
Ожидаемые значения на ss14-wega: 24 находки в 7 файлах, `FALSE POSITIVES: 0`, полный проход 40 секунд.

`MeasureStates` — одиннадцатый, и сторож у него не про контент: он падает, если проверенных
значений оказалось меньше десяти тысяч, потому что так выглядит ослепший обход, а не изменившийся
чекаут — первая версия блочного парсера нашла 949 значений вместо 14000 и не сказала об этом ничего.
Сами находки печатаются: три на ss14-wega, все настоящие.

`MeasureGuidebook` — седьмой: он читает `ServerInfo/**/*.xml` отгружаемым `RobustGuidebook.references`
и сверяет каждый id с видами из индекса прототипов. Падает он не на мёртвой ссылке (это ошибка
контента), а на `WRONG KIND` — id, который в чекауте объявлен, но другим видом: так выглядел бы
атрибут, приписанный не к тому виду, и плагин красил бы ошибкой работающую страницу. Ожидаемые
значения на ss14-wega: 296 документов, 1240 embed'ов, 726 разных id, `DEAD: 1`, `WRONG KIND: 0`,
полный проход 25 мс.

`MeasureTypedIds` — восьмой, и он стережёт не индекс, а право писать в контент: правило
«имя ключа означает прототип» читается из отгружаемого `RobustDataFieldIndex`, а потом под каждым
принятым именем пересчитываются значения прототипов. Падает он на имени, у которого значений-не-id
больше, чем id, — так выглядит имя, означающее что-то другое там, где оно на самом деле стоит
(`drawdepth`, `mask`, `airBlockedDirection` — на них он и сработал, когда сериализаторы отбирались
по суффиксу). Строка `NOT AN ID` печатает остаток: одиночные чужие значения под именем, которое
в остальном сплошь id, — нормальное состояние, а не регресс. Ожидаемые значения на ss14-wega:
922 имени объявлены прототипом хоть где-то, 110 отвергнуты как неоднозначные, 812 принято,
23879 значений, `NOT AN ID: 0`, полный проход 12 секунд. Вторая половина замера стережёт сторону
значений словарей тем же способом и падает на `NOT AN ID IN A MAPPING`: 29 имён, 5 неоднозначных,
24 принято, 4072 значения, ноль чужих.

`MeasureFlags` — девятый: он строит карту «тег → enum» из `[FlagsFor]` тем же правилом, что и
движок, и падает, если у тега оказалось два enum'а (такой чекаут просто не загрузится). Значения
он читает отгружаемым `RobustValidation.unknownMembers`, но только под теми именами ключей, которые
во всём чекауте означают флаги, — и то, что под этот фильтр не проходят как раз `layer` и `mask`,
и есть его главный вывод: строка `VALUES A BY-NAME RULE WOULD PAINT RED` считает цену правила по
имени. Ожидаемые значения на ss14-wega: 5 enum'ов на 6 тегов, ключи `airBlockedDirection`, `flag`,
`layer`, `mask`, `visMask`, из них однозначны два, 31 проверенное значение, отвергнутых 0,
`PAINT RED: 122`.

`MeasureCodeLinks` — десятый: он читает `.cs` отгружаемым `RobustCodeLinks.links` и сверяет каждый
литерал с диском и с индексом id. Падает он на `UNKNOWN KIND` — классе из `ProtoId<X>`, который
чекаут не объявляет прототипом: сегодня таких нет, и появление любого означает, что карта «класс →
вид» перестала отвечать, а вместе с ней тихо отключилась проверка, которая держит rename в стороне
от одноимённого id другого вида. Остальное — дело контента и печатается: `WRONG KIND` (1 —
`Rev`), `DEAD` (3 живых промаха плюс 69 фикстур, объявленных строкой внутри теста) и `DEAD PATH`
(33). Ожидаемые значения на ss14-wega: 1133 пути, 1127 id и 62 состояния (ни одного без кадра),
полный проход 900 мс.

Мель этого замера стоит помнить, потому что она из нашей же предметной области: обход блочного
списка требовал отступа **глубже** ключа, а SS14 пишет элементы **на уровне** ключа — ровно то,
что делает `RobustSequenceIndentHandler`. Из 17 объявлений `airBlockedDirection` в контенте так
записаны 14, и замер честно показывал три значения вместо двадцати девяти.

Отдельная мель того же замера: `MeasureRequired` искал родителя по id **без учёта вида**
(`byId.putIfAbsent`), а `MaterialCloth` объявлен и сущностью, и `cargoProduct`, `GasValve` — тремя
видами сразу. Родителем сущности оказывался `cargoProduct`, и семь обязательных полей выглядели
незаданными. В плагине этого нет: `RobustRequiredFields` обходит **все** объявления id и мержит их
ключи. Замер теперь берёт в родители только `entity`.

`MeasureDeadLocale` — пятый сторож, и самый дорогой: он собирает семь источников использований
(значения прототипов, литералы `.cs`, ссылки `.ftl`, привязки XAML, атрибуты гайдбука, `ent-*` от
движка, аффиксы из кода и `localizedDataset`), а потом перечитывает весь чекаут и падает, если
ключ, объявленный мёртвым, где-нибудь написан вне комментария. Комментарии режутся до поиска: 1436
кандидатов упоминаются только в закомментированном YAML, и без этого проверка объявила бы их живыми.
Ожидаемые значения на ss14-wega: 56280 сообщений, мёртвых 1750, `SPELLED OUT SOMEWHERE: 0`, проход
около трёх минут — единственный замер, который не укладывается в секунды.
...
16 отвержений, которые он печатает и которых в IDE не бывает: `on`/`broken` — ключи
...
- [x] Предупреждение о неиспользуемых сообщениях `.ftl`: семь источников использований, 1750 мёртвых
      из 56280 (3%), ложных ноль
...
- [x] Ctrl+клик прямо с литерала в `.cs`. Alt+F7 и Shift+F6 оттуда невозможны: оба действия
      подменены на `BackendDelegatingAction` и уходят в ReSharper выше любого EP платформы
- [x] Мёртвые ключи локализации по соседям: 24 находки в 7 файлах, ложных ноль
- [x] Гайдбук: свой корень (без него ни индекса, ни записи), ссылки на прототипы из embed'ов,
      проверка вида, 476 ключей перестали краситься мёртвыми
- [x] Типизированные ссылки на id: 812 имён ключей из regex-индекса вместо пяти зашитых,
      23879 значений получили Ctrl+клик, Find Usages и rename; вид берётся у индекса, а не
      у бэкенда, иначе rename переписывал бы разный набор файлов от прогретости кэша
- [x] Флаги `layer:`/`mask:`: `FlagSerializer<TTag>` через обратный поиск `[FlagsFor]` — 1336
      значений под пятью полями, 43 члена `CollisionGroup`; значения кастомного сериализатора
      сравниваются с учётом регистра
- [x] Id в значениях словаря: `protoValueField:` — ключ маппинга придумывает автор (`mask:` под
...
- [x] Значения по умолчанию датафилда в ховере: инициализатор из синтаксиса, секция `Default`
      ...
      оставлен
- [x] Переходы из `.cs` в контент и обратно: Ctrl+клик с литерала ведёт в файл ресурса (1133 пути,
      972 резолвятся), в объявление прототипа (1127 id, 783 разных) и в кадр `.rsi` (62 состояния,
      мёртвых нет), а Alt+F7 и Shift+F6 по id
      с YAML-стороны теперь видят и правят литералы `.cs` — из C# эти два действия невозможны
      (см. запись про `BackendDelegatingAction`), поэтому обратная половина и делает мост
      двусторонним. Сканер литералов взят готовый (`RobustLocaleUsageIndex.stringRanges`);
      правило для id требует тип на той же строке, а для rename — ещё и совпадение вида
      из `ProtoId<X>`. Сторож — `MeasureCodeLinks`
- [x] `RobustYamlContext` больше не тянет вверх: `isLocalizationValue` переехала к своему
      единственному вызывающему (`RobustYamlReferenceContributor`), и два импорта наверх
      (`inspection.RobustValidation`, `lookup.RobustLocalization`) сняты. В корне остался чистый
      обход дерева — то, ради чего он и тестируется голым `ParsingTestCase`
- [x] Литерал C# как полноценная ссылка: `psi.referenceContributor language="C#"` на
      `CSharpStringLiteralExpression` (Ctrl+клик и подчёркивание), автокомплит через `getVariants`
      с подрезанным `DUMMY_IDENTIFIER`, ховер через `platform.backend.documentation.targetProvider`
      — Rider уступает его, когда бэкенд не отдал сессию. Alt+F7 и Shift+F6 из C# остаются
      невозможными (`BackendDelegatingAction`), подсветка мёртвых ссылок в C# тоже: она едет
      с бэкенда через `rider.grave.*`
- [x] Полировка редактора: спрайт в гуттере (кадр только названный — состояние, одиночный `.png`
      или `icon.png`), инлеи с именем сущности и текстом сообщения, сворачивание объявлений и
      компонентов, Structure View через точку YAML, живые шаблоны `ent`/`proto`/`comp` с макросом
      дефиса. По ходу выяснено, почему шаблоны не попадали в список (контекст спрашивался у копии
      файла автокомплита) и почему их отбирал `stopHere` (порядок с `liveTemplates` не определён)
- [x] Иконки сущностей в автокомплите id: `LookupElementRenderer` вместо `withIcon` — 14274
      элемента у вида `entity`, а рендерер зовут только для видимых строк
- [x] Проверка `state:` по унаследованному `sprite:`: 11520 значений проверяются против `.rsi`,
      5507 из них через спрайт предка. Уровень — ошибка (`Logger.ErrorS` плюс `ErrorNode` у
      валидатора, роняющий линтер в CI); молчим рядом с `layers:` (149), без спрайта в области
      (1449) и без `.rsi` в чекауте (197). `MeasureStates`: три находки, все настоящие, ложных ноль
- [x] Ctrl+H — иерархия прототипов (EP `typeHierarchyProvider`): узел это объявление id, рёбра из
      `parent:` в обе стороны, вниз — через `RobustYamlReferenceIndex`, отдельного индекса не нужно
