...
Тесты остались одним пакетом: их 20, каждый назван по тому, что проверяет, и раскладывать их
...
| `RobustCodeDeclarationHandler` | Ctrl+клик с литерала `.cs` в файл ресурса и в объявление прототипа |
...
Alt+F7 не работает: каретка в открытом файле никогда не даёт `PsiFile`. «Dummy»-парсер C#
(`CSharpDummyParser`) строит настоящее дерево токенов, поэтому `ActionCallPolicy.getElement` возвращает
`psiFile.findElementAt(offset)` — токен, а не файл. Своих `usageTargetProvider` и
...
попавшимся `getParentOfType`. Такие вещи закрываются тестами: `./gradlew :test`, 205 случаев,
5 секунд.
